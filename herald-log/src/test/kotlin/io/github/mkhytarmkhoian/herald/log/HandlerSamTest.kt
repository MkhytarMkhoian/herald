...
class HandlerSamTest {
    ...
    fun `A lambda satisfies both handler interfaces`() = runTest {
        val service = LogAnalyticsTrackerService(
            eventTrackerFactory = CompositeLogEventTrackerFactory(
                LogEventTrackerFactory { Resolution.Claimed(LogEventTracker { calls += "track:${it.name}" }) },
            ),
            propertySetterFactory = CompositeLogPropertySetterFactory(
                LogPropertySetterFactory { Resolution.Claimed(LogPropertySetter { calls += "set:${it.name}" }) },
            ),
        )
